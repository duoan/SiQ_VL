...
def estimate_memory_usage(
    ...
    # Vision activations
    vision_activations = (
        batch_size * num_patches * vision_hidden * bytes_per_param
    ) / (1024 ** 3)
    
    # Projected image embeddings: [batch, num_patches, llm_hidden]
    image_embeds = (
        batch_size * num_patches * llm_hidden * bytes_per_param
    ) / (1024 ** 3)
    ...
    text_embeds = (
        batch_size * text_seq_len * llm_hidden * bytes_per_param
    ) / (1024 ** 3)
    ...
    combined_embeds = (
        batch_size * actual_seq_len * llm_hidden * bytes_per_param
    ) / (1024 ** 3)
    ...
    if gradient_checkpointing:
        ...
        llm_hidden_states = (
            batch_size * actual_seq_len * llm_hidden * stored_layers * bytes_per_param
        ) / (1024 ** 3)
        ...
        attention_memory_per_layer = (
            batch_size * num_heads * actual_seq_len * actual_seq_len * bytes_per_param
        ) / (1024 ** 3)
        ...
        ffn_memory = (
            batch_size * actual_seq_len * llm_hidden * 4 * stored_layers * bytes_per_param
        ) / (1024 ** 3)
        ...
        llm_hidden_states = (
            batch_size * actual_seq_len * llm_hidden * num_layers * bytes_per_param
        ) / (1024 ** 3)
        
        # Attention matrices for all layers
        attention_memory = (
            batch_size * num_heads * actual_seq_len * actual_seq_len * num_layers * bytes_per_param
        ) / (1024 ** 3)
        
        # FFN activations
        ffn_memory = (
            batch_size * actual_seq_len * llm_hidden * 4 * num_layers * bytes_per_param
        ) / (1024 ** 3)
    ...
    return {
        ...
        "activations": {
            ...
            "total_activations": (
                vision_activations + image_embeds + text_embeds + 
                combined_embeds + llm_activations
            ),
        },
        "training": {
...
def print_memory_report(memory_breakdown: Dict, config: Dict):
    ...
    print(f"  Patches Before Shuffle: {seq_info['num_patches_before_shuffle']} ({seq_info['patches_per_dim']}x{seq_info['patches_per_dim']})")
    print(f"  Patches After Shuffle: {seq_info['num_patches_after_shuffle']} (reduction: {seq_info['reduction_factor']:.1f}x)")
